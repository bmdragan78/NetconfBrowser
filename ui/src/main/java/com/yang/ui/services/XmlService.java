...
import com.yang.ui.services.model.YangDataSchemaNode;
...
import com.yang.ui.services.xml.LocationAnnotator;
...
//			throw new Exception("Root element can only be <rpc> or <notification>");
	...
	private String buildBitsValue(TypeDefinition type, String defaultValue) throws Exception {
		...
		if(type instanceof BitsTypeDefinition) {
			...
			Collection<? extends Bit> bitList = ((BitsTypeDefinition) type).getBits();
	    	List<Bit> bitListSorted = bitList.stream().sorted((p1, p2)->new Long(p1.getPosition().longValue()).compareTo(p2.getPosition().longValue())).collect(Collectors.toList());
	    	genValue = "";
	...
	private String buildValue(Element leafElement, TypeDefinition type, ModuleIdentifier moduleId, TreeItem<SchemaItem> rootTreeItem, YangModule yangModule) throws Exception {//add other int types!!!
		String genValue = null;
		...
		if(type instanceof BinaryTypeDefinition) {
			if( ((BinaryTypeDefinition) type).getLengthConstraint().isPresent()) {
				LengthConstraint  lengthConstraint = ((BinaryTypeDefinition) type).getLengthConstraint().get();
				RangeSet<Integer> rangeSet = lengthConstraint.getAllowedRanges();
				Set<Range<Integer>> rs = rangeSet.asRanges();
	    		if(rs != null && rs.size() > 0) {
	    			Range<Integer> range = (new ArrayList<Range<Integer>>(rs)).get(0);
					int minChars =range.lowerEndpoint().intValue();
					if(minChars == 0)
			    		minChars++;
	    	...
	    	if(((DecimalTypeDefinition) type).getRangeConstraint().isPresent()) {
	    		RangeConstraint<Decimal64>  rangeConstraint = ((DecimalTypeDefinition) type).getRangeConstraint().get();
	    		RangeSet<Decimal64> rangeSet = rangeConstraint.getAllowedRanges();
	    		Set<Range<Decimal64>> rs = rangeSet.asRanges();
	    		if(rs != null && rs.size() > 0) {
	    			Range<Decimal64> range = (new ArrayList<Range<Decimal64>>(rs)).get(0);
		    		int minValue =range.lowerEndpoint().intValue();
			    	genValue = "" + minValue;
			...
			IdentitySchemaNode identitySchema = ((IdentityrefTypeDefinition) type).getIdentities().stream().findFirst().get();
...
//	    	List<RangeConstraint>  rangeConstraint = ((Int8TypeDefinition) type).getRangeConstraints();//size >=1 Always; includes base constraints && and defaults to [0..maxint]
//	    	int minValue = rangeConstraint.get(0).getMin().intValue();
//	    	genValue = "" + minValue;
	    	if(((Int8TypeDefinition) type).getRangeConstraint().isPresent()) {
	    		RangeConstraint<Byte>  rangeConstraint = ((Int8TypeDefinition) type).getRangeConstraint().get();
	    		RangeSet<Byte> rangeSet = rangeConstraint.getAllowedRanges();
	    		Set<Range<Byte>> rs = rangeSet.asRanges();
	    		if(rs != null && rs.size() > 0) {
	    			Range<Byte> range = (new ArrayList<Range<Byte>>(rs)).get(0);
		    		int minValue =range.lowerEndpoint().intValue();
			    	genValue = "" + minValue;
	    		}
	    	}
	    }else if(type instanceof StringTypeDefinition) {//ignore Regex Pattern Constraints
	    	if( ((StringTypeDefinition) type).getLengthConstraint().isPresent()) {
				LengthConstraint  lengthConstraint = ((StringTypeDefinition) type).getLengthConstraint().get();
				RangeSet<Integer> rangeSet = lengthConstraint.getAllowedRanges();
				Set<Range<Integer>> rs = rangeSet.asRanges();
	    		if(rs != null && rs.size() > 0) {
	    			Range<Integer> range = (new ArrayList<Range<Integer>>(rs)).get(0);
	    	...
	    	if(((Uint8TypeDefinition) type).getRangeConstraint().isPresent()) {
	    		RangeConstraint<Uint8>  rangeConstraint = ((Uint8TypeDefinition) type).getRangeConstraint().get();
	    		RangeSet<Uint8> rangeSet = rangeConstraint.getAllowedRanges();
	    		Set<Range<Uint8>> rs = rangeSet.asRanges();
	    		if(rs != null && rs.size() > 0) {
	    			Range<Uint8> range = (new ArrayList<Range<Uint8>>(rs)).get(0);
		    		int minValue =range.lowerEndpoint().intValue();
			    	genValue = "" + minValue;
	    		}
	    	}
...
//    		String xpath = schemaSrv.getXpath(targetXPath, prefixMap);
	    	PathExpression targetXPath = ((LeafrefTypeDefinition) type).getPathStatement();
    		String xpath = schemaSrv.getXpath(targetXPath, prefixMap);
	...
	private SchemaItem validateTreeRec(Element element, Map<String, Stack<SchemaItem>> namespacePathMap, TreeItem<SchemaItem> treeItem, MyNamespaceContext nsCtx, Document doc, List<ErrorItem> errorList, String queryName) throws Exception {//fix unique !!!!
//		if(contor == 0 && pathStack.size() == 0 && !element.equals("rpc") && !element.equals("notification"))//root nodes missing
//			throw new Exception("Root element can only be <rpc> or <notification>");
		SchemaItem schemaItem = null;
		TreeItem<SchemaItem> elementTreeItem = null;
...
//	  					Collection<? extends UniqueEffectiveStatement> uniqueDef = childListItem.getUniqueDef();
//	  					for(UniqueEffectiveStatement unique : uniqueDef) {
//	  						value = "";
//	  						String uniquePath = "";
////	  						Collection<Relative> relList = unique.getTag();
////	  						for(Relative rel : relList) {//c1/index c1/name
//	  						
//	  						Collection<Relative> relList = unique.
//	  						for(Relative rel : relList) {//c1/index c1/name
//	  							Node childListNode = dataElem.get(index);
//	  							
...
//	  					}
	...
	private boolean validateLeafValue(TypeDefinition unionType, YangSchemaNode schemaNode, String value, TreeItem<SchemaItem> rootTreeItem, MyNamespaceContext nsCtx, YangModule yangModule, List<ErrorItem> errorList, String queryName, LocationData locationData) throws Exception {//fix types !!!
