...
import com.yang.ui.services.model.CapabilityItem;
import com.yang.ui.services.model.ErrorItem;
import com.yang.ui.services.model.ErrorTypeEnum;
import com.yang.ui.services.model.FeatureItem;
...
import com.yang.ui.services.model.YangAny;
import com.yang.ui.services.model.YangAugmentation;
import com.yang.ui.services.model.YangAugmentationHeader;
import com.yang.ui.services.model.YangChoice;
import com.yang.ui.services.model.YangChoiceCase;
import com.yang.ui.services.model.YangContainer;
import com.yang.ui.services.model.YangDataHeader;
import com.yang.ui.services.model.YangDeviate;
import com.yang.ui.services.model.YangDeviation;
import com.yang.ui.services.model.YangDeviationHeader;
import com.yang.ui.services.model.YangExtension;
import com.yang.ui.services.model.YangExtensionHeader;
import com.yang.ui.services.model.YangFeature;
import com.yang.ui.services.model.YangFeatureHeader;
import com.yang.ui.services.model.YangFile;
import com.yang.ui.services.model.YangGrouping;
import com.yang.ui.services.model.YangGroupingHeader;
import com.yang.ui.services.model.YangIdentity;
import com.yang.ui.services.model.YangIdentityHeader;
import com.yang.ui.services.model.YangLeaf;
import com.yang.ui.services.model.YangLeafList;
import com.yang.ui.services.model.YangList;
import com.yang.ui.services.model.YangModule;
import com.yang.ui.services.model.YangNode;
import com.yang.ui.services.model.YangNotification;
import com.yang.ui.services.model.YangNotificationHeader;
import com.yang.ui.services.model.YangRepo;
import com.yang.ui.services.model.YangRpc;
import com.yang.ui.services.model.YangRpcHeader;
import com.yang.ui.services.model.YangSchemaNode;
import com.yang.ui.services.model.YangType;
import com.yang.ui.services.model.YangTypeHeader;
...
public class SchemaService  {
	...
	public TreeItem<SchemaItem> getSchemaTreeRoot(String filterType, String filterText, boolean isAscending, boolean isCategories){//called from UI
		...
		try {
			...
			CrossSourceStatementReactor reactor = RFC7950Reactors.vanillaReactorBuilder().build();
			...
			for(YangFile yangFile : yangFiles){
				...
				try {
						...
						final YangStatementStreamSource yangFileSource = YangStatementStreamSource.create(YangTextSchemaSource.forPath(yangFile.getFilePath()));
						...
						for(YangFile yangImport : yangImports){
							...
							final YangStatementStreamSource yangImportSource = YangStatementStreamSource.create(YangTextSchemaSource.forPath(yangImport.getFilePath()));
						...
						CrossSourceStatementReactor.BuildAction reactorAction = reactor.newBuild();
						reactorAction.addSources(yangSources);
						final SchemaContext schemaContext = reactorAction.buildEffective();

						//build only OD module defined in yangFile
						Collection<? extends Module> modulesOD = schemaContext.getModules();
						...
						for(Module moduleOD : modulesOD)
							moduleIdMap.put(moduleOD.getNamespace().toString(), new ModuleIdentifier(moduleOD.getName(), moduleOD.getNamespace().toString(), moduleOD.getQNameModule().getRevision().isPresent() ? moduleOD.getQNameModule().getRevision().get().toString() : "", moduleOD.getPrefix()));

						Collection<? extends Module> moduleODSet = schemaContext.findModules(XMLNamespace.of(yangFile.getModuleNamespace()));
						...
						YangModule yangModule = new YangModule( moduleOD.getName(), moduleOD.getNamespace().toString(), moduleOD.getPrefix(), moduleOD.getQNameModule().getRevision().isPresent() ? moduleOD.getQNameModule().getRevision().get().toString() : "", 
								moduleOD.getOrganization().orElse(""), moduleOD.getContact().orElse(""), moduleOD.getDescription().orElse(""), isDataContainer, moduleOD.getImports());
						...
						Collection<? extends DataSchemaNode> rootNodesOD = moduleOD.getChildNodes();
	...
	private YangSchemaNode buildTreeRec(Object nodeOD, ModuleIdentifier moduleId, boolean isCategories) {
		...
		if(nodeOD instanceof ChoiceSchemaNode) {//implements both DataNodeContainer & DataSchemaNode
			...
			InputSchemaNode  inputOD = ((RpcDefinition)nodeOD).getInput();
			...
			OutputSchemaNode  outputOD = ((RpcDefinition)nodeOD).getOutput();
...
//			}
	...
	private YangSchemaNode buildNode(Object nodeOD, ModuleIdentifier moduleId) {
		...
		if(nodeOD instanceof SchemaNode) {
			...
			xpath = getXpath( SchemaPath.of( Absolute.of(((SchemaNode)nodeOD).getQName())), moduleId);
		}
		//data statements
		...
		}else if(nodeOD instanceof InputSchemaNode) {
			InputSchemaNode containerOD = (InputSchemaNode)nodeOD;
			yangNode = new YangContainer(containerOD, xpath, moduleId);
		}else if(nodeOD instanceof OutputSchemaNode) {
			OutputSchemaNode containerOD = (OutputSchemaNode)nodeOD;
			yangNode = new YangContainer(containerOD, xpath, moduleId);
		...
		}else if(nodeOD instanceof CaseSchemaNode) {
			CaseSchemaNode choiceCaseOD = (CaseSchemaNode)nodeOD;
...
//			String targetPathPrefix = getTargetPathPrefix(deviationOD.getTargetPath());
			String targetPath = getTargetPathModule(SchemaPath.of(deviationOD.getTargetPath()));
			String targetPathPrefix = getTargetPathPrefix(SchemaPath.of(deviationOD.getTargetPath()));
			...
			String groupingPath = getTargetPathModule(SchemaPath.of( Absolute.of(((SchemaNode)usesOD.getSourceGrouping()).getQName())));
			String groupingPathPrefix = getTargetPathPrefix(SchemaPath.of( Absolute.of(((SchemaNode)usesOD.getSourceGrouping()).getQName())));
			yangNode = new YangUses(usesOD, groupingPath, groupingPathPrefix, moduleId);
		}else if(nodeOD instanceof AugmentationSchemaNode) {				
			AugmentationSchemaNode augOD = (AugmentationSchemaNode)nodeOD;
			String targetPath = getTargetPathModule(SchemaPath.of(augOD.getTargetPath()));
			String targetPathPrefix = getTargetPathPrefix(SchemaPath.of(augOD.getTargetPath()));
...
//	public String getXpath(RevisionAwareXPath targetXPath, Map<String, String> prefixMap) {//buildValue & validateLeafValue from XmlService
...
//		return xpathBuf.substring(0, xpathBuf.length() - 1).toString();
//	}
	public String getXpath(PathExpression targetXPath, Map<String, String> prefixMap) {//buildValue & validateLeafValue from XmlService
		String prefix = getPrefixFrom(targetXPath);
		String moduleName = prefixMap.get(prefix);
	...
	private String getSimplePathFrom(PathExpression targetXPath) {
	...
	private String getPrefixFrom(PathExpression targetXPath) {
		String prefix = "";
		try {
...
//	}
	...
	public static String getUniques(Collection<? extends UniqueEffectiveStatement> uniquesSet) {
...
//		return mustBuf.toString();
//	}
	public static String getMust(Collection<? extends MustDefinition> mustSet) {
		StringBuilder mustBuf = new StringBuilder();
		
		if(mustSet != null) {
			for(MustDefinition mustDef : mustSet) {
				QualifiedBound mustXpath = mustDef.getXpath();
				mustBuf.append(mustXpath.toString() + File.separator);
			}
		}
		return mustBuf.toString();
...
//			SchemaPath schemaPath = ((SchemaNode)nodeOD).getPath();
			//String groupingPathPrefix = getTargetPathPrefix(SchemaPath.of( Absolute.of(((SchemaNode)usesOD.getSourceGrouping()).getQName())));
			SchemaPath schemaPath = SchemaPath.of( Absolute.of(((SchemaNode)nodeOD).getQName()));
				
			if(nodeOD instanceof CaseSchemaNode || nodeOD instanceof ChoiceSchemaNode)
...
//						break;
//					if(schemaNodeParent instanceof CaseSchemaNode || schemaNodeParent instanceof ChoiceSchemaNode) {
