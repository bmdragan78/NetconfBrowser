...
import static com.yang.ui.codearea.fxmisc.richtext.model.TwoDimensional.Bias.*;
...
import com.yang.ui.codearea.fxmisc.richtext.model.TwoDimensional.Position;
...
class GenericStyledAreaBehavior {
    ...
    private void handleFirstPrimaryPress(MouseEvent e) {
        ...
        System.out.println("handleFirstPrimaryPress X " + e.getX() + " Y " + e.getY() + " char " + ( hit.getCharacterIndex().isPresent() ? hit.getCharacterIndex().getAsInt() : "N/A"));
        ...
        }else if(e.getX() <= 50) {//Ignore clicks on line number label
        	// press inside selection
            dragSelection = DragState.POTENTIAL_DRAG;
            dragNewSelection = DragState.NO_DRAG;
        }else {
